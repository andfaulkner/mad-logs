...
const isomorphic_styles_1 = require("./src/isomorphic-styles");
exports.isomorphicStyles = isomorphic_styles_1.isomorphicStyles;
/************************************* IMPORT PROJECT MODULES *************************************/
const theming_1 = require("./src/theming");
exports.logMarkers = theming_1.logMarkers;
const build_file_tag_string_1 = require("./src/build-file-tag-string");
/**************************** CONDITIONAL IMPORT/EXPORT BY ENVIRONMENT ****************************/
exports.buildFileTag = (isNode)
    ? build_file_tag_string_1.buildFileTagForCli
 ...
 */
const logFactory = (config = defConfig) => verifyConfig(config, (conf) => function buildLog(fileName, opts = defLogOpts) {
    const logLevelNum = getLogVal(conf.logLevel);
    const fileTag = (isNode)
        ? build_file_tag_string_1.buildFileTagForCli(fileName, opts.style)
     ...
     */
    const logMethodFactory = (levelNum, out = basicLog) => {
        return (...strs) => {
    ...
    log.wtf = logMethodFactory(8, (...strs) => {
        (isNode)
            ? console.error('\n', red(bgWhite(`[! DANGER: FATAL ERROR !] ${fileTag}`)), ' :: ', ...strs, '\n')
            : console.error(fileTag, ': ', '%c[! DANGER: FATAL ERROR !]', 'color: red;', ':: ', ...strs);
    });
