    ...
    return function buildLog(fileName, opts) {
        ...
        log.error = logMethodFactory(7, function () {
            ...
            if (isNode) {
                ...
                console.error.apply(console, [fileTag, ' [ERROR] '].concat(strs));
            }
            return strs[strs.length - 1];
        });
        ...
        log.wtf = logMethodFactory(8, function () {
            ...
            if (isNode) {
                ...
                console.error.apply(console, [fileTag, " " + wtfTag + " "].concat(strs));
            }
            return strs[strs.length - 1];
        });
...
function warnLogOut(fileTag) {
    return function () {
        ...
        if (isNode) {
            ...
            console.warn.apply(console, [fileTag, ' [WARNING] '].concat(strs));
        }
        return strs[strs.length - 1];
    };
}
