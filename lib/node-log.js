    ...
    var logObj = Object.assign(logObjFnBase, {
        ...
        silly: function () {
            ...
            if (env_var_helpers_1.isSilly)
                console.log.apply(console, [fTAG + " "].concat(argsToLog));
        },
        sillyError: function () {
            var argsToLog = [];
            for (var _i = 0; _i < arguments.length; _i++) {
            ...
            Object.keys(_this);
            if (env_var_helpers_1.isSilly)
                console.error.apply(console, [fTAG + " "].concat(argsToLog));
        },
        verbose: function () {
            var argsToLog = [];
            for (var _i = 0; _i < arguments.length; _i++) {
        ...
        warn: function () {
            ...
            if (env_var_helpers_1.isWarn)
                console.warn.apply(console, [fTAG + " "].concat(argsToLog));
        },
        error: function () {
            var argsToLog = [];
            for (var _i = 0; _i < arguments.length; _i++) {
...
exports.nodeLogFactory = function (TAG) {
    ...
    Object.assign(logObjBoundDeep, {
        fn: function (fnName) {
            ...
            var builtLogObjFactory = logObjFactory(TAG, fnName);
            // For some reason fn keeps getting attached.
            if (builtLogObjFactory.fn)
                delete builtLogObjFactory.fn;
